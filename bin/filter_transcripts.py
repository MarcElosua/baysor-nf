...
def main():
    ...
    # Build output path
    fn = '_'.join(["X"+str(args.min_x)+"-"+str(args.max_x), "Y"+str(args.min_y)+"-"+str(args.max_y), "filtered_transcripts.csv"])
    out = '/'.join([args.out, fn])
    print("Saving filtered transcripts: " + out)
    # Output filtered transcripts to CSV
    filtered_frame.to_csv(out,
                          index=False,
                          encoding = 'utf-8')
...
def parse_args():
    ...
    requiredNamed.add_argument('-out',
                               required = True,
                               help="The path to store the filtered transcripts " +
